...
disp('Adding up-sinks')
upMets = setdiff(metsToModify, metNoSinkUp);
upMetsG = strcat(upMets, '_G');
upMetsL = strcat(upMets, '_L');
l_umg = length(upMetsG);
l_uml = length(upMetsL);
upRxns = cellfun(@(x) strcat('sink_', x, '_up'), upMets, 'UniformOutput', false);
l_ur = length(upRxns);
upStoich = [-diag(ones(1, l_umg)); -diag(ones(1, l_uml))];
...
%          -> mets, stoich, rev, lb, ub, obj. coef, subsys, gpr, geneNameList, systNameList, checkduplicate
%end

disp('Adding down-sinks')
downMets = setdiff(metsToModify, metNoSinkDown);
downMetsG = strcat(downMets, '_G');
downMetsL = strcat(downMets, '_L');
l_dmg = length(downMetsG);
l_dml = length(downMetsL);
downRxns = cellfun(@(x) strcat('sink_', x, '_down'), downMets, 'UniformOutput', false);
l_dr = length(downRxns);
downStoich = [diag(ones(1, l_dmg)); diag(ones(1, l_dml))];
...
disp('Reconciling data and fluxes')
[m, ~] = size(uFBAmodel.S);
[~, n] = size(model.S);
