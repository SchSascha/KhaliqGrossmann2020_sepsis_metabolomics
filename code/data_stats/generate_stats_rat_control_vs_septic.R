...
#Set path
out_dir <- "../../results/data_stats_rat_control_vs_septic/"
...
rat_sepsis_data$group[grep(pattern = "septic", x = rat_sepsis_data$group)] <- "septic"
...
rat_plasma_sig_diff_res <- rat_sig_diffs_along_time(subset(rat_sepsis_data, material == "plasma"), corr_fdr = T)
...
rat_heart_sig_diff_res <- rat_sig_diffs_along_time(subset(rat_sepsis_data, material == "heart"), corr_fdr = T)
...
rat_liver_sig_diff_res <- rat_sig_diffs_along_time(subset(rat_sepsis_data, material == "liver"), corr_fdr = T)
...
###Survival-regardent
rat_sepsis_data_normal_C_conc_metab_cov <- cov(subset(rat_sepsis_data_normal, group == "control", metab_sel), use = "pairwise.complete.obs")
rat_sepsis_data_normal_S_conc_metab_cov <- cov(subset(rat_sepsis_data_normal, group == "septic", metab_sel), use = "pairwise.complete.obs")
rat_sepsis_data_normal_C_conc_pheno_cov <- cov(subset(rat_sepsis_data_normal, group == "control", pheno_sel), use = "pairwise.complete.obs")
rat_sepsis_data_normal_S_conc_pheno_cov <- cov(subset(rat_sepsis_data_normal, group == "septic", pheno_sel), use = "pairwise.complete.obs")
...
###Survival-regardent
rat_sepsis_data_normal_C_grouped_conc_metab_cov <- cov(subset(rat_sepsis_data_normal_grouped, group == "control", group_metab_sel), use = "pairwise.complete.obs")
rat_sepsis_data_normal_S_grouped_conc_metab_cov <- cov(subset(rat_sepsis_data_normal_grouped, group == "septic", group_metab_sel), use = "pairwise.complete.obs")
rat_sepsis_data_normal_C_grouped_conc_pheno_cov <- cov(subset(rat_sepsis_data_normal_grouped, group == "control", group_pheno_sel), use = "pairwise.complete.obs")
rat_sepsis_data_normal_S_grouped_conc_pheno_cov <- cov(subset(rat_sepsis_data_normal_grouped, group == "septic", group_pheno_sel), use = "pairwise.complete.obs")
...
heatmaply(x = x[,-1:-4], row_side_colors = x[c("group")], file = paste0(out_dir, "rat_normal_metab.png"), main = "", key.title = "Concentration\n(normalized)", showticklabels = TRUE)
x <- na.omit(rat_sepsis_data_normal[,c(1:4, pheno_sel)])
heatmaply(x = x[,-1:-4], row_side_colors = x[c("group")], file = paste0(out_dir, "rat_normal_pheno.png"), main = "", key.title = "Concentration\n(normalized)", showticklabels = c(TRUE, FALSE), column_text_angle = 90)
x <- na.omit(rat_sepsis_data_normal_grouped[,c(1:4, group_metab_sel)])
heatmaply(x = x[,-1:-4], row_side_colors = x[c("group")], file = paste0(out_dir, "rat_normal_grouped_metab.png"), main = "", key.title = "Concentration\n(normalized)", showticklabels = c(T, F))
rm("x")

##Rat, cluster-heatmap, coarse grouped metabolites
x <- na.omit(rat_sepsis_data_normal_metab_cov)
heatmaply(x = x[,-1:-4], row_side_colors = x[c("group")], col_side_colors = x["material"], file = paste0(out_dir, "rat_normal_grouped_metab_cov.png"), main = "", key.title = "Cov", showticklabels = FALSE)
x <- na.omit(rat_sepsis_data_normal_pheno_cov)
heatmaply(x = x[,-1:-4], row_side_colors = x[c("group")], file = paste0(out_dir, "rat_normal_grouped_pheno_cov.png"), main = "", key.title = "Cov", showticklabels = FALSE)
rm("x")
...
heatmaply(x = x[,-1:-4], row_side_colors = x[c("group")], file = paste0(out_dir, "rat_normal_grouped_pheno_cov.png"), main = "")
rm("x")
...
heatmaply(x = x[,-1:-4], row_side_colors = x[c("group")], file = paste0(out_dir, "rat_normal_metab_cor.png"), main = "")
x <- na.omit(rat_sepsis_data_normal_pheno_cor)
heatmaply(x = x[,-1:-4], row_side_colors = x[c("group")], file = paste0(out_dir, "rat_normal_pheno_cor.png"), main = "")
rm("x")
...
heatmaply(x = x[,-1:-5], row_side_colors = x[c("group")], file = paste0(out_dir, "rat_normal_grouped_metab_cor.png"), main = "")
x <- na.omit(rat_sepsis_data_normal_grouped_pheno_cor)
heatmaply(x = x[,-1:-5], row_side_colors = x[c("group")], file = paste0(out_dir, "rat_normal_grouped_pheno_cor.png"), main = "")
rm("x")
...
x <- na.omit(rat_sepsis_data_normal_C_conc_metab_cov)
heatmaply(x = x, file = paste0(out_dir, "rat_normal_C_conc_metab_cov.png"), key.title = "Cov")
...
x <- rat_sepsis_data_normal_C_conc_pheno_cov
heatmaply(x = x, file = paste0(out_dir, "rat_normal_C_conc_pheno_cov.png"), key.title = "Cov")
...
x <- rat_sepsis_data_normal_C_grouped_conc_metab_cov
heatmaply(x = x, file = paste0(out_dir, "rat_normal_C_grouped_conc_metab_cov.png"), key.title = "Cov")
...
x <- na.omit(rat_sepsis_data_normal_C_grouped_conc_pheno_cov)
heatmaply(x = x, file = paste0(out_dir, "rat_normal_C_grouped_conc_pheno_cov.png"), key.title = "Cov")
...
r_time_course_sig_diff_dat <- subset(rat_sepsis_data_long_form_sig, material == "plasma" & variable %in% rat_plasma_sig_t_class)
...
r_time_course_sig_diff_plot <- ggplot(na.omit(r_time_course_sig_diff_dat), aes_string(x = "`time point`", y = "value", group = "group", color = "group")) +
  ...
  theme_bw()
ggsave(plot = r_time_course_sig_diff_plot, filename = "rat_metab_plasma_time_course_sig_diff.png", path = out_dir, width = 10, height = 9, units = "in")

##rat, liver, metabolite concentration time course, only metabolites with p-val < 0.05 at any time point
r_time_course_sig_diff_dat <- subset(rat_sepsis_data_long_form_sig, material == "liver" & variable %in% rat_liver_sig_t_class)
...
r_time_course_sig_diff_plot <- ggplot(na.omit(r_time_course_sig_diff_dat), aes_string(x = "`time point`", y = "value", group = "group", color = "group")) +
  facet_wrap(facets = ~ variable, ncol = 6, nrow = ceiling(length(rat_liver_time_sig_t_diff)/6)) +
  stat_summary(fun.ymin = "min", fun.ymax = "max", fun.y = "mean", geom = "line") +
  stat_summary(fun.ymin = "min", fun.ymax = "max", fun.y = "mean", size = 0.5) +
  geom_point(data = r_time_course_sigs, mapping = aes(x = Time, y = value), shape = 8, inherit.aes = FALSE, size = 0.8) +
  geom_text(data = r_time_course_group, mapping = aes(x = Time, y = value, label = text), inherit.aes = FALSE, size = 3.2) +
  ...
  theme_bw()
ggsave(plot = r_time_course_sig_diff_plot, filename = "rat_metab_liver_time_course_sig_diff.png", path = out_dir, width = 8, height = 3, units = "in")
